...
const ADL_READ_ONLY_ELEMENTS = new Set([
  "adl.nav.request_valid.continue",
  "adl.nav.request_valid.previous",
])
...
const ACTIONABLE_NAV_REQUESTS = new Set([
  "continue",
  "previous",
  "exit",
  "exitAll",
...
function setDynamicCount(values, name) {
  ...
  if (interactionMatch) {
    const count = Math.max(
      Number(values["cmi.interactions._count"] || 0),
      Number(interactionMatch[1]) + 1,
    )
  ...
  if (objectiveMatch) {
    const count = Math.max(
      Number(values["cmi.objectives._count"] || 0),
      Number(objectiveMatch[1]) + 1,
    )
  ...
  if (commentMatch) {
    const count = Math.max(
      Number(values["cmi.comments_from_learner._count"] || 0),
      Number(commentMatch[1]) + 1,
    )
...
export function createScormRuntimeApi({
  ...
  const initialize = (argument) => {
    ...
    const masteryScore =
      values[is2004 ? "cmi.scaled_passing_score" : "cmi.student_data.mastery_score"] || ""
